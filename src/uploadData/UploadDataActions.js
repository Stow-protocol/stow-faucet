...
const uploadingToIpfs = (message) => ({
  type: UPLOADING_TO_IPFS,
  isLoading: true,
  message
})

const uploadingToLinnia = (message) => ({
  type: UPLOADING_TO_LINNIA,
  isLoading: true,
  message
})
...
export function uploadData (file, public_key, metadata) {
  ...
  return async (dispatch) => {
    ...
    fileReader.onloadend = async (e) => {
      ...
      //Encrypt
      try {
        dispatch(uploadingToIpfs("Encrypting..."))
      ...
      //Upload to IPFS
      try {
        dispatch(uploadingToIpfs("Uploading to IPFS..."))
      ...
      try {
        dispatch(uploadingToLinnia("Uploading to Linnia..."))
